...
# Might increase this again to 1000
...
def gen_data(simlen: int):
    ...
    for workload_id, workload in enumerate(workloads):
        ...
        if elapsed_seconds is None:
            print(f"Failed for workload_id: {workload_id}, num_cells: {num_cells}, simulator_id: {simulator_id}, simlen: {simlen}, randseed: {randseed}")
            continue
    ...
    for num_cells in num_cells_list:
        for simulator_id, _ in enumerate(simulators):
            if f"{num_cells}_{simulator_id}_{simlen}" not in elapsed_times_per_pair_gen:
                print(f"Expected key {num_cells}_{simulator_id}_{simlen} not found in elapsed_times_per_pair_gen")
                print("Available keys:")
                for key in elapsed_times_per_pair_gen:
                    print(key)
...
with open('eval_execperf.json', 'w') as f:
