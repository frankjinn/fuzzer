...
from pyentropy.toggleanalysis import toggle_coverage
...
def testtoggle_wrapper(workload, min_num_cells, max_num_cells, simlen: int):
    timeout = 20 # Maybe increase to 200 or so

    while True:
        with concurrent.futures.ThreadPoolExecutor() as executor:
            # Submit the function to the executor
            future = executor.submit(__testtoggle, workload, min_num_cells, max_num_cells, simlen)
...
def __testtoggle(workload, min_num_cells, max_num_cells, simlen: int):
    try:
        random.seed(workload)
        workdir = os.path.join(PATH_TO_WORKDIR_ROOT, f"tmp/obj_dir_example_testtoggle_{workload}")
        ...
        incremental_coverage = toggle_coverage(fuzzerstate.get_tracefile(), simlen)
        ...
        print(f"CalledProcessError exception in workload {workload}")
        return None
